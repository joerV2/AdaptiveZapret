...
static std::map<int, bool> selectedRows;
static int lastSelectedRowId = -1;

static std::atomic<bool> g_GlobalStopRequested = false;

// Для ручного добавления стратегии
static bool showAddStrategyPopup = false;
static char newStrategyBuffer[4096] = "";
static int manualAddDomainId = -1;
static std::string manualAddDomainHostname = "";

// Состояние GameFilter для каждого домена (кэш)
static std::map<int, bool> gameFilterState;

// Удалённый сканер (клиент)
static std::string g_RemoteScannerIP = "127.0.0.1";
static char g_RemoteScannerIPBuf[64] = "";
static int g_RemoteScannerPort = 0;
static char g_RemoteScannerPortBuf[16] = "";
static bool g_RemoteScannerConnected = false;
static bool g_RemoteScannerConnectionError = false;
static SOCKET g_RemoteScannerSocket = INVALID_SOCKET;
static std::mutex g_RemoteScannerMutex;
static std::thread g_RemoteScannerThread;
static std::atomic<bool> g_RemoteScannerStopFlag(false);
...
// -------------------------------------------------------------------
// Нормализация путей к .bin файлам в стратегиях
// -------------------------------------------------------------------
static std::string NormalizeStrategyPaths(const std::string& strategy) {
    std::regex binPathRegex(R"((/cygdrive/[a-zA-Z]/[^\s"]+zapret-win-bundle-master/blockcheck/zapret/)(files/fake/[^\s"]+\.bin))", std::regex::icase);
    std::string result = strategy;
    std::smatch match;
    if (std::regex_search(strategy, match, binPathRegex)) {
        std::string relativePath = match[2].str();
        result = std::regex_replace(strategy, binPathRegex, relativePath);
    }
    std::regex winPathRegex(R"(([a-zA-Z]:\\[^"]+zapret-win-bundle-master\\blockcheck\\zapret\\)(files\\fake\\[^"]+\.bin))", std::regex::icase);
    if (std::regex_search(strategy, match, winPathRegex)) {
        std::string relativePath = match[2].str();
        std::replace(relativePath.begin(), relativePath.end(), '\\', '/');
        result = std::regex_replace(strategy, winPathRegex, relativePath);
    }
    return result;
}
...
static void StopWinDivertService() {
    ...
    if (schSCManager) {
        SC_HANDLE schService = OpenServiceA(schSCManager, "WinDivert", SERVICE_STOP | SERVICE_QUERY_STATUS);
        if (schService) {
            SERVICE_STATUS status;
            if (QueryServiceStatus(schService, &status) && status.dwCurrentState == SERVICE_RUNNING) {
                ControlService(schService, SERVICE_CONTROL_STOP, &status);
                for (int i = 0; i < 10 && status.dwCurrentState != SERVICE_STOPPED; ++i) Sleep(100);
            }
...
static bool EnsureWinDivertDriver() {
    ...
    fs::path driverPath = exeDir / "WinDivert64.sys";
    if (!fs::exists(driverPath)) {
        driverPath = exeDir / "zapret-win-bundle-master" / "zapret-winws" / "WinDivert64.sys";
        if (!fs::exists(driverPath)) {
            MessageBoxW(NULL, L"WinDivert64.sys не найден", L"Ошибка", MB_ICONERROR);
            CloseServiceHandle(schSCManager);
            return false;
        }
    }
...
// -------------------------------------------------------------------
// Работа с реестром (DoH, ZAPRET, встроенный DNS браузеров, автозапуск)
// -------------------------------------------------------------------
...
static bool LoadZapretState() {
    ...
    if (RegOpenKeyExA(HKEY_CURRENT_USER, "Software\\AdaptiveZapret", 0, KEY_QUERY_VALUE, &hKey) == ERROR_SUCCESS) {
        ...
        RegCloseKey(hKey);
        OutputDebugStringA(("LoadZapretState: " + std::to_string(val == 1) + "\n").c_str());
    }
    else {
        OutputDebugStringA("LoadZapretState: key not found, default false\n");
    }
    return val == 1;
}
...
static void SetAutoStart(bool enable) {
    ...
    if (RegOpenKeyExA(HKEY_CURRENT_USER,
        ...
        if (enable) {
            char path[MAX_PATH];
            GetModuleFileNameA(NULL, path, MAX_PATH);
            RegSetValueExA(hKey, "AdaptiveZapret", 0, REG_SZ, (BYTE*)path, (DWORD)strlen(path) + 1);
        }
        else {
            RegDeleteValueA(hKey, "AdaptiveZapret");
        }
        RegCloseKey(hKey);
    }
    // Сохраняем настройку в реестр приложения
    HKEY hAppKey;
    if (RegCreateKeyExA(HKEY_CURRENT_USER, "Software\\AdaptiveZapret", 0, NULL,
        REG_OPTION_NON_VOLATILE, KEY_SET_VALUE, NULL, &hAppKey, NULL) == ERROR_SUCCESS) {
        DWORD val = enable ? 1 : 0;
        RegSetValueExA(hAppKey, "AutoStart", 0, REG_DWORD, (BYTE*)&val, sizeof(val));
        RegCloseKey(hAppKey);
    }
}

static bool LoadAutoStartState() {
    HKEY hKey;
    DWORD val = 0;
    DWORD size = sizeof(val);
    if (RegOpenKeyExA(HKEY_CURRENT_USER, "Software\\AdaptiveZapret", 0, KEY_QUERY_VALUE, &hKey) == ERROR_SUCCESS) {
        RegQueryValueExA(hKey, "AutoStart", NULL, NULL, (BYTE*)&val, &size);
        RegCloseKey(hKey);
    }
    return val == 1;
}
...
// -------------------------------------------------------------------
// RemoteScannerClient (удалённый сканер)
// -------------------------------------------------------------------
class RemoteScannerClient {
public:
    static bool Connect(const std::string& ip, int port) {
        std::lock_guard<std::mutex> lock(g_RemoteScannerMutex);
        if (g_RemoteScannerConnected) return true;
        ...
        if (connect(sock, (sockaddr*)&addr, sizeof(addr)) == SOCKET_ERROR) {
            closesocket(sock);
            g_RemoteScannerConnectionError = true;
            return false;
        }

        char buffer[16];
        int recvLen = recv(sock, buffer, sizeof(buffer) - 1, 0);
        if (recvLen <= 0 || strncmp(buffer, "ALIVE", 5) != 0) {
            closesocket(sock);
            g_RemoteScannerConnectionError = true;
            return false;
        }

        g_RemoteScannerSocket = sock;
        g_RemoteScannerConnected = true;
        g_RemoteScannerConnectionError = false;
        g_RemoteScannerIP = ip;
        g_RemoteScannerPort = port;
        return true;
    }

    static void Disconnect() {
        std::lock_guard<std::mutex> lock(g_RemoteScannerMutex);
        if (!g_RemoteScannerConnected) return;
        if (g_RemoteScannerSocket != INVALID_SOCKET) {
            closesocket(g_RemoteScannerSocket);
            g_RemoteScannerSocket = INVALID_SOCKET;
        }
        g_RemoteScannerConnected = false;
        g_RemoteScannerConnectionError = false;
    }

    static bool IsConnected() {
        std::lock_guard<std::mutex> lock(g_RemoteScannerMutex);
        return g_RemoteScannerConnected;
    }

    static bool HasError() {
        std::lock_guard<std::mutex> lock(g_RemoteScannerMutex);
        return g_RemoteScannerConnectionError && !g_RemoteScannerConnected;
    }

    static std::vector<std::string> ScanDomain(const std::string& domain, const std::string& level) {
        SOCKET localSock = INVALID_SOCKET;
        {
            std::lock_guard<std::mutex> lock(g_RemoteScannerMutex);
            if (!g_RemoteScannerConnected || g_RemoteScannerSocket == INVALID_SOCKET) return {};
            localSock = g_RemoteScannerSocket;
            std::string cmd = "SCAN " + domain + " " + level + "\n";
            if (send(localSock, cmd.c_str(), (int)cmd.size(), 0) == SOCKET_ERROR) {
                if (g_RemoteScannerConnected) {
                    closesocket(g_RemoteScannerSocket);
                    g_RemoteScannerSocket = INVALID_SOCKET;
                    g_RemoteScannerConnected = false;
                }
        ...
        std::string line;
        while (true) {
            int received = recv(localSock, buffer, sizeof(buffer) - 1, 0);
            if (received <= 0) {
                std::lock_guard<std::mutex> lock(g_RemoteScannerMutex);
                if (g_RemoteScannerConnected && g_RemoteScannerSocket == localSock) {
                    closesocket(g_RemoteScannerSocket);
                    g_RemoteScannerSocket = INVALID_SOCKET;
                    g_RemoteScannerConnected = false;
                }
            ...
            while ((pos = line.find('\n')) != std::string::npos) {
                ...
                line.erase(0, pos + 1);
                if (part == "END") {
                    for (auto& s : strategies) s = NormalizeStrategyPaths(s);
                    return strategies;
                }
                if (part == "CANCELLED") return { "_CANCELLED_" };
                if (part == "BUSY") return {};
                strategies.push_back(part);
            }
        }
    }
    ...
    static void CancelScan() {
        ...
        send(g_RemoteScannerSocket, cmd.c_str(), (int)cmd.size(), 0);
    }
};
...
public:
    ...
    static void CancelCurrent(const std::string& domain) {
        if (RemoteScannerClient::IsConnected()) {
            RemoteScannerClient::CancelScan();
            return;
        }
        ...
        if (it != activeScans.end()) {
            ...
            if (it->second.hJob) TerminateJobObject(it->second.hJob, 0);
            else if (it->second.hProcess) TerminateProcess(it->second.hProcess, 0);
        }
        else {
    ...
    static void CancelAll() {
        ...
        std::lock_guard<std::mutex> lock(scanMutex);
        for (auto& pair : activeScans) {
            pair.second.cancelRequested = true;
            if (pair.second.hJob) TerminateJobObject(pair.second.hJob, 0);
            else if (pair.second.hProcess) TerminateProcess(pair.second.hProcess, 0);
        }
        activeScans.clear();
    }
    ...
    static std::vector<std::string> RunBlockCheck(const std::string& domain) {
        if (RemoteScannerClient::IsConnected()) {
            return RemoteScannerClient::ScanDomain(domain, g_ScanLevel);
        }

        if (g_GlobalStopRequested) return { "_CANCELLED_" };
        ...
        if (!fs::exists(curlPath)) curlPath = cygwinBin / "curl.exe";
        ...
        if (!fs::exists(bashPath) || !fs::exists(blockcheckScript)) {
            MessageBoxA(NULL, "bash.exe or blockcheck.sh not found", "Error", MB_ICONERROR);
            std::lock_guard<std::mutex> lock(scanMutex);
        ...
        std::error_code ec;
        fs::remove(logPath, ec); fs::remove(answersPath, ec);
        ...
        if (!fs::exists(cygwinTmp)) fs::create_directory(cygwinTmp);
        std::string createTmpCmd = "\"" + bashPath.string() + "\" -c \"mkdir -p /tmp\"";
        STARTUPINFOA si = { sizeof(si) };
        PROCESS_INFORMATION pi = { 0 };
        si.dwFlags = STARTF_USESHOWWINDOW; si.wShowWindow = SW_HIDE;
        CreateProcessA(NULL, (LPSTR)createTmpCmd.c_str(), NULL, NULL, FALSE, 0, NULL, cygwinBin.string().c_str(), &si, &pi);
        WaitForSingleObject(pi.hProcess, INFINITE); CloseHandle(pi.hProcess); CloseHandle(pi.hThread);

        std::string scanLevelCmd = g_ScanLevel;
        if (scanLevelCmd != "quick" && scanLevelCmd != "standard") scanLevelCmd = "quick";
        ...
        {
            std::lock_guard<std::mutex> lock(scanMutex);
            if (ctx->cancelRequested || g_GlobalStopRequested) {
                if (ctx->hJob) CloseHandle(ctx->hJob);
                activeScans.erase(domain);
        ...
        if (!CreateProcessA(NULL, (LPSTR)command.c_str(), NULL, NULL, FALSE, 0, NULL, cygwinBin.string().c_str(), &si, &pi)) {
            DWORD err = GetLastError();
        ...
        bool alreadyCancelled = false;
        {
            ...
            if (ctx->cancelRequested || g_GlobalStopRequested) {
                ...
                if (ctx->hJob) AssignProcessToJobObject(ctx->hJob, pi.hProcess);
            }
        }
        ...
        if (!alreadyCancelled) {
            while (true) {
                ...
                if (ctx->cancelRequested || g_GlobalStopRequested) {
                    alreadyCancelled = true;
                    if (ctx->hJob) TerminateJobObject(ctx->hJob, 0);
                    else TerminateProcess(pi.hProcess, 0);
                    break;
                }
            }
        ...
        while (std::getline(logFile, line)) {
            debug << line << "\n";
            if (line.find("* SUMMARY") != std::string::npos) inSummary = true;
            else if (inSummary && line.find("winws --") != std::string::npos) {
                if (line.find("working without bypass") != std::string::npos) continue;
                size_t pos = line.find("winws --");
                if (pos != std::string::npos) {
                    std::string strategy = line.substr(pos);
                    while (!strategy.empty() && isspace(strategy.back())) strategy.pop_back();
...
public:
    ...
    static std::set<std::string> userCancelledDomains;
    static std::mutex userCancelMutex;
    ...
    static void OnNewDomain(const std::string& domain, bool forceCheck = false) {
        ...
        std::thread([domain]() {
            ...
            {
                std::lock_guard<std::mutex> lock(userCancelMutex);
                if (userCancelledDomains.count(domain)) {
                    userCancelledDomains.erase(domain);
                    wasCancelled = true;
                }
            }
            if (wasCancelled) {
            ...
            if (!strategies.empty() && strategies[0] == "_CANCELLED_") {
                {
                    ...
                    if (dom.id != -1 && dom.status != u8"проверка отменена") {
                        Database::UpdateDomainStatus(domain, u8"проверка отменена");
                    }
                }
                newDataFlag = true;
                return;
    ...
    static void CancelScan(const std::string& domain) {
        {
            std::lock_guard<std::mutex> lock(userCancelMutex);
            userCancelledDomains.insert(domain);
        }
        ...
        }
        {
            std::lock_guard<std::mutex> dbLock(databaseMutex);
            auto dom = Database::GetDomainByHostname(domain);
            if (dom.id != -1 && (dom.status == u8"заблокирован (проверка)" || dom.status == u8"стратегий не найдено" || dom.status == u8"чистый")) {
                Database::UpdateDomainStatus(domain, u8"проверка отменена");
            }
        }
        newDataFlag = true;
    ...
    static void GlobalStopAllScans() {
        g_GlobalStopRequested = true;
        {
            std::lock_guard<std::mutex> lock(userCancelMutex);
            for (const auto& pair : Scanner::activeScans) userCancelledDomains.insert(pair.first);
            std::lock_guard<std::mutex> pLock(pendingMutex);
            for (const auto& d : pendingDomains) userCancelledDomains.insert(d);
        }
        Scanner::CancelAll();
        {
            std::lock_guard<std::mutex> lock(domainsMutex);
        ...
        {
            std::lock_guard<std::mutex> dbLock(databaseMutex);
            auto allDomains = Database::GetAllDomains();
            for (const auto& d : allDomains) {
                if (d.status == u8"заблокирован (проверка)") {
                    Database::UpdateDomainStatus(d.hostname, u8"проверка отменена");
                }
            }
        }
        newDataFlag = true;
        std::thread([]() {
            while (true) {
                {
                    std::lock_guard<std::mutex> lock(Scanner::scanMutex);
                    if (Scanner::activeScans.empty()) break;
                }
                Sleep(200);
            }
            {
                std::lock_guard<std::mutex> lock(DomainManager::pendingMutex);
                if (!DomainManager::pendingDomains.empty()) DomainManager::pendingDomains.clear();
            }
            g_GlobalStopRequested = false;
            OutputDebugStringA("GlobalStopAllScans: all scans stopped, flag reset.\n");
            }).detach();
    }

private:
    static bool IsReachable(const std::string& hostname) {
        auto rec = Database::GetDomainByHostname(hostname);
        if (rec.id != -1 && rec.status == u8"проверка отменена") return false;
    ...
    static SOCKET CreateSocket(const std::string& hostname, int port) {
        ...
        if (getaddrinfo(hostname.c_str(), std::to_string(port).c_str(), &hints, &res) != 0) return INVALID_SOCKET;
        ...
        if (ret <= 0) { closesocket(sock); freeaddrinfo(res); return INVALID_SOCKET; }
    ...
    static bool TcpConnect(const std::string& hostname, int port) {
        SOCKET sock = CreateSocket(hostname, port);
        if (sock != INVALID_SOCKET) { closesocket(sock); return true; }
        return false;
    }
};
...
std::set<std::string> DomainManager::userCancelledDomains;
std::mutex DomainManager::userCancelMutex;
...
private:
    ...
    static void SnifferThread() {
        handle = WinDivertOpen("(udp.DstPort == 53 or tcp.DstPort == 53)", WINDIVERT_LAYER_NETWORK, 0, WINDIVERT_FLAG_SNIFF);
        if (handle == INVALID_HANDLE_VALUE) { OutputDebugStringA("Sniffer: WinDivertOpen failed\n"); return; }
        char packet[4096];
        WINDIVERT_ADDRESS addr;
        UINT packetLen;
        while (running) {
            if (!WinDivertRecv(handle, packet, sizeof(packet), &packetLen, &addr)) { if (!running) break; continue; }
...
public:
    ...
    static void Stop() {
        running = false;
        if (handle != INVALID_HANDLE_VALUE) { WinDivertClose(handle); handle = INVALID_HANDLE_VALUE; }
...
public:
    ...
    static bool Start() {
        OutputDebugStringA("ZapretController::Start called\n");
        if (isRunning) {
            OutputDebugStringA("ZapretController::Start: already running\n");
            return true;
        }
        if (!GenerateDomainLists()) {
            OutputDebugStringA("ZapretController::Start: GenerateDomainLists failed\n");
            return false;
        }
        fs::path exeDir = GetExeDirectory();
        fs::path zapretDir = exeDir / "zapret-win-bundle-master";
        ...
        if (!fs::exists(winwsExe)) {
            OutputDebugStringA(("ZapretController::Start: winws.exe not found at " + winwsExe.string() + "\n").c_str());
        ...
        for (const auto& entry : fs::directory_iterator(strategiesDir)) {
            ...
            if (name.rfind("domains_", 0) == 0 && name.size() > 4 && name.substr(name.size() - 4) == ".txt") {
                ...
                if (!strategyParams.empty()) block += " " + strategyParams;
                blocks.push_back(block);
            }
        }
        if (blocks.size() == 1) cmdLine += blocks[0];
        else if (blocks.size() > 1) {
            for (size_t i = 0; i < blocks.size(); ++i) {
                if (i == 0) cmdLine += blocks[i];
                else cmdLine += " --new" + blocks[i];
            }
        }
        ...
        if (!CreateProcessA(NULL, (LPSTR)cmdLine.c_str(), NULL, NULL, FALSE, 0, NULL, zapretDir.string().c_str(), &si, &pi)) {
            OutputDebugStringA("ZapretController::Start: CreateProcess failed\n");
            return false;
        }
        isRunning = true;
...
static bool showAddStrategyModal = false;
static char manualStrategyBuffer[4096] = "";
static std::string manualStrategyDomain;
static int manualStrategyDomainId = -1;

// -------------------------------------------------------------------
// Функция обновления списка доменов
// -------------------------------------------------------------------
void RefreshDomainList() {
    ...
    gameFilterState.clear();
    for (auto& d : domains) {
        auto strategies = Database::GetStrategiesForDomain(d.id);
        strategies.insert(strategies.begin(), u8"---без обхода---");
        ...
        gameFilterState[d.id] = d.game_filter;
    }
}
    ...
    for (auto& d : domains) {
        ...
        if (newIdx != originalStrategyIdx[d.id]) {
            ...
            if (newIdx >= 0 && newIdx < (int)stratList.size()) {
                std::string newStrat = (newIdx == 0) ? "" : stratList[newIdx];
                Database::UpdateActiveStrategy(d.hostname, newStrat);
            }
        }
        bool gf = gameFilterState[d.id];
        if (gf != d.game_filter) {
            Database::UpdateGameFilter(d.hostname, gf);
        }
    }
    for (auto& d : domains) originalStrategyIdx[d.id] = selectedStrategyIdx[d.id];
    ...
    if (ImGui::BeginPopupModal(u8"Домен(ы)", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
        ...
        if (ImGui::Button(u8"OK")) {
            if (strlen(newDomainBuffer) > 0) {
                ...
                std::string domain;
                while (ss >> domain) {
                    if (!domain.empty()) std::thread([domain]() { DomainManager::OnNewDomain(domain, true); }).detach();
                }
                newDomainBuffer[0] = '\0';
...
bool IsAnyScanActive() {
    std::lock_guard<std::mutex> lock(DomainManager::domainsMutex);
    if (!scanningDomains.empty()) return true;
    std::lock_guard<std::mutex> pLock(DomainManager::pendingMutex);
    return !DomainManager::pendingDomains.empty();
}

void ShowManualStrategyPopup() {
    if (!showAddStrategyModal) return;
    ImGui::OpenPopup(u8"Добавление стратегии");
    if (ImGui::BeginPopupModal(u8"Добавление стратегии", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
        ImGui::TextWrapped(u8"Стратегии добавляются в формате \"winws --флаги\", рекомендуется использовать именно те стратегии, что были найдены при ручном запуске blockcheck, стратегии из zapret-discord-youtube возможно необходимо будет адаптировать, не копируйте их без изменений, это может всё сломать.");
        ImGui::InputText(u8" ", manualStrategyBuffer, IM_ARRAYSIZE(manualStrategyBuffer));
        if (ImGui::Button(u8"OK")) {
            std::string strategy = manualStrategyBuffer;
            if (!strategy.empty() && manualStrategyDomainId != -1) {
                Database::AddStrategy(manualStrategyDomainId, strategy);
                Database::UpdateActiveStrategy(manualStrategyDomain, strategy);
                RefreshDomainList();
            }
            showAddStrategyModal = false;
            manualStrategyBuffer[0] = '\0';
            manualStrategyDomainId = -1;
            manualStrategyDomain.clear();
            ImGui::CloseCurrentPopup();
        }
        ImGui::SameLine();
        if (ImGui::Button(u8"Отмена")) {
...
void RenderUI() {
    ...
    if (dragging && ImGui::IsItemActive() && ImGui::IsMouseDragging(ImGuiMouseButton_Left)) {
        ...
        SetWindowPos(g_hWnd, NULL, dragStartWindowRect.left + (currentPos.x - dragStartPos.x), dragStartWindowRect.top + (currentPos.y - dragStartPos.y), 0, 0, SWP_NOSIZE | SWP_NOZORDER);
    }
    ...
    if (ImGui::BeginTabBar("MainTabs")) {
        if (ImGui::BeginTabItem(u8"Управление")) {
            ...
            if (ImGui::Button(zapretState ? u8"Остановить ZAPRET" : u8"Запустить ZAPRET", ImVec2(220, 0))) {
                if (zapretState) ZapretController::Stop(); else ZapretController::Start();
                zapretState = ZapretController::IsRunning();
            }
            if (!zapretState) { ImGui::PushItemFlag(ImGuiItemFlags_Disabled, true); ImGui::PushStyleVar(ImGuiStyleVar_Alpha, 0.5f); }
            if (ImGui::Button(u8"Перезапуск ZAPRET", ImVec2(220, 0))) {
                ZapretController::Restart();
                zapretState = ZapretController::IsRunning();
            }
            if (!zapretState) { ImGui::PopItemFlag(); ImGui::PopStyleVar(); }
            if (ImGui::IsItemHovered() && !zapretState) ImGui::SetTooltip(u8"ZAPRET не запущен");

            if (ImGui::Button(snifferState ? u8"Выключить сниффер" : u8"Включить сниффер", ImVec2(220, 0))) {
                if (snifferState) Sniffer::Stop(); else Sniffer::Start();
            ...
            if (ImGui::Button(u8"Очистить DNS-кэш", ImVec2(220, 0))) FlushDnsCache();
            ImGui::Separator();
            ...
            ImGui::TextWrapped(u8"Запустите внешний сканер на чистой системе для получения рабочих стратегий даже во время пользования VPN или других методов обхода.");
            ImGui::Separator();
            ImGui::SetNextItemWidth(120.0f);
            if (ImGui::InputText(u8"IP-адрес", g_RemoteScannerIPBuf, sizeof(g_RemoteScannerIPBuf))) {
                g_RemoteScannerIP = g_RemoteScannerIPBuf;
            }
            ImGui::SameLine();
            ImGui::SetNextItemWidth(80.0f);
            if (ImGui::InputText(u8"Порт", g_RemoteScannerPortBuf, sizeof(g_RemoteScannerPortBuf), ImGuiInputTextFlags_CharsDecimal)) {
                int newPort = atoi(g_RemoteScannerPortBuf);
                if (newPort > 0 && newPort < 65536) g_RemoteScannerPort = newPort;
                else if (strlen(g_RemoteScannerPortBuf) == 0) g_RemoteScannerPort = 0;
            }
            ...
            if (ImGui::Button(g_RemoteScannerConnected ? u8"Отключиться" : u8"Подключиться")) {
                if (!g_RemoteScannerConnected) RemoteScannerClient::Connect(std::string(g_RemoteScannerIP), g_RemoteScannerPort);
                else RemoteScannerClient::Disconnect();
            }
            if (!canToggleRemote) { ImGui::PopItemFlag(); ImGui::PopStyleVar(); }
            ImGui::SameLine();
            if (g_RemoteScannerConnected) ImGui::TextColored(ImVec4(0, 1, 0, 1), u8"Подключено");
            else if (RemoteScannerClient::HasError()) ImGui::TextColored(ImVec4(1, 0, 0, 1), u8"Ошибка подключения");
            else ImGui::TextColored(ImVec4(0, 0, 1, 1), u8"Отключено");
            ImGui::Separator();

            // Автозапуск
            bool autoStart = g_AutoStartEnabled;
            if (ImGui::Checkbox(u8"Автозапуск при включении системы", &autoStart)) {
                g_AutoStartEnabled = autoStart;
        ...
        if (ImGui::BeginTabItem(u8"Список доменов")) {
            ...
            ImGui::Text(u8"Поиск:");
            ImGui::SameLine();
            ImGui::InputText(u8"##search", searchBuffer, IM_ARRAYSIZE(searchBuffer));
            ImGui::SameLine();
            ImGui::Text(u8"Режим сканирования:");
            ImGui::SameLine();
            ImGui::SetNextItemWidth(100.0f);
            if (ImGui::BeginCombo(u8"##scanlevel", g_ScanLevel.c_str(), 0)) {
                if (ImGui::Selectable("quick", g_ScanLevel == "quick")) g_ScanLevel = "quick";
                if (ImGui::Selectable("standard", g_ScanLevel == "standard")) g_ScanLevel = "standard";
                ImGui::EndCombo();
            }
            if (ImGui::IsItemHovered()) ImGui::SetTooltip(u8"quick — быстрый, standard — полный (дольше)");
            ...
            if (!DomainManager::pendingDomains.empty()) {
                ...
                for (size_t i = 0; i < DomainManager::pendingDomains.size(); ++i) {
                    ...
                    if (ImGui::SmallButton(u8"Удалить")) {
                        ...
                        { std::lock_guard<std::mutex> lock2(DomainManager::domainsMutex); scanningDomains.erase(dom); }
                        RefreshDomainList();
                        ImGui::PopID();
                        break;
                    }
                ...
                if (ImGui::Button(u8"Очистить очередь")) {
                    std::lock_guard<std::mutex> lock(DomainManager::pendingMutex);
                    for (const auto& d : DomainManager::pendingDomains) { std::lock_guard<std::mutex> lock2(DomainManager::domainsMutex); scanningDomains.erase(d); }
            ...
            auto disabledIfSniffer = [snifferOn]() {
                if (snifferOn) { ImGui::PushItemFlag(ImGuiItemFlags_Disabled, true); ImGui::PushStyleVar(ImGuiStyleVar_Alpha, 0.5f); }
            };
            auto enabledIfSniffer = [snifferOn]() {
                if (snifferOn) { ImGui::PopItemFlag(); ImGui::PopStyleVar(); }
            };
            ...
            if (unsavedChanges) { ImGui::PushItemFlag(ImGuiItemFlags_Disabled, true); ImGui::PushStyleVar(ImGuiStyleVar_Alpha, 0.5f); }
            if (ImGui::Button(u8"Составить настройки стратегий")) {
                SaveAndGenerate();
                ImGui::OpenPopup(u8"ListsGenerated");
            }
            if (unsavedChanges) { ImGui::PopItemFlag(); ImGui::PopStyleVar(); }
            if (ImGui::BeginPopupModal(u8"ListsGenerated", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
                ImGui::Text(u8"Списки стратегий обновлены.\nДля применения изменений перезапустите ZAPRET.");
                if (ImGui::Button(u8"OK")) ImGui::CloseCurrentPopup();
                ImGui::EndPopup();
            }
            ImGui::SameLine();

            if (ImGui::Button(u8"Остановить все проверки")) {
                DomainManager::GlobalStopAllScans();
                RefreshDomainList();
            }
            ImGui::SameLine();
            ...
            if (!snifferOn && unsavedChanges) {
                ...
                if (ImGui::Button(u8"Сохранить и применить")) {
                    SaveAndGenerate();
                    ImGui::OpenPopup(u8"SaveApplied");
                }
                if (ImGui::BeginPopupModal(u8"SaveApplied", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
                    ImGui::Text(u8"Списки стратегий обновлены.\nДля применения изменений перезапустите ZAPRET.");
                    if (ImGui::Button(u8"OK")) ImGui::CloseCurrentPopup();
                    ImGui::EndPopup();
                }
            ...
            if (ImGui::BeginTable("table", 6, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollY)) {
                ...
                ImGui::TableSetupColumn(u8"GameFilter", ImGuiTableColumnFlags_WidthFixed, 80.0f);
                ...
                for (auto& d : domains) {
                    ...
                    bool checked = selectedRows[d.id];
                    if (ImGui::Checkbox(("##chk" + std::to_string(d.id)).c_str(), &checked)) {
                        if (ImGui::GetIO().KeyShift && lastSelectedRowId != -1) {
                            int start = lastSelectedRowId;
                            int end = d.id;
                            if (start > end) std::swap(start, end);
                            for (int i = start; i <= end; ++i) selectedRows[i] = checked;
                        }
                        else selectedRows[d.id] = checked;
                        lastSelectedRowId = d.id;
                    }
                    ...
                    { std::lock_guard<std::mutex> lock(DomainManager::domainsMutex); isScanning = (scanningDomains.find(d.hostname) != scanningDomains.end()); }
                    if (isScanning) {
                        ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1), u8"Идёт проверка...");
                    }
                    else {
                        ...
                        if (!strategies.empty()) {
                            ...
                            if (ImGui::Combo(("##strat_" + std::to_string(d.id)).c_str(), &current, items.data(), (int)items.size())) {
                                if (!snifferOn) { selectedStrategyIdx[d.id] = current; if (current != originalStrategyIdx[d.id]) unsavedChanges = true; }
                            }
                            if (snifferOn) { ImGui::PopItemFlag(); ImGui::PopStyleVar(); }
                        }
                        else ImGui::TextDisabled(u8"нет стратегий");
                    }

                    ImGui::TableSetColumnIndex(4);
                    if (!snifferOn && !isScanning) {
                        bool gf = gameFilterState[d.id];
                        if (ImGui::Checkbox(("##gf" + std::to_string(d.id)).c_str(), &gf)) {
                            gameFilterState[d.id] = gf;
                            unsavedChanges = true;
                        }
                        if (ImGui::IsItemHovered()) ImGui::SetTooltip(u8"Применять выбранную стратегию к игровым портам");
                    }
                    ...
                    if (!snifferOn) {
                        ...
                        { std::lock_guard<std::mutex> pLock(DomainManager::pendingMutex); isPending = (std::find(DomainManager::pendingDomains.begin(), DomainManager::pendingDomains.end(), d.hostname) != DomainManager::pendingDomains.end()); }
                        if (isScanning || isPending) {
                            if (ImGui::Button((u8"Отмена##" + std::to_string(d.id)).c_str())) {
                            ...
                            ImGui::SameLine();
                            if (ImGui::Button((u8"Проверить##" + std::to_string(d.id)).c_str())) {
                                g_PendingActionDomain = d.hostname;
                                g_PendingAction = PendingAction::Recheck;
                            }
                            if (ImGui::IsItemHovered()) ImGui::SetTooltip(u8"Повторная проверка");
                            ImGui::SameLine();
                            if (ImGui::Button((u8"+##" + std::to_string(d.id)).c_str())) {
                                manualStrategyDomain = d.hostname;
                                manualStrategyDomainId = d.id;
                                manualStrategyBuffer[0] = '\0';
                                showAddStrategyModal = true;
                            }
                            if (ImGui::IsItemHovered()) ImGui::SetTooltip(u8"Добавить стратегию вручную");
                        }
                    }
                    else ImGui::TextDisabled(u8"---");
                }
            ...
            ImGui::Separator();
            ImGui::Text(u8"С отмеченными:");
            ImGui::SameLine();

            if (scanningActive) { ImGui::PushItemFlag(ImGuiItemFlags_Disabled, true); ImGui::PushStyleVar(ImGuiStyleVar_Alpha, ImGui::GetStyle().Alpha * 0.5f); }
            if (ImGui::Button(u8"Повторная проверка")) ImGui::OpenPopup(u8"ConfirmMassRecheck");
            if (ImGui::BeginPopupModal(u8"ConfirmMassRecheck", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
                ImGui::Text(u8"Запустить повторную проверку для выбранных доменов?");
                if (ImGui::Button(u8"Да")) {
                    for (auto& pair : selectedRows) {
                        if (pair.second) {
                            for (const auto& d : domains) {
                                if (d.id == pair.first) { DomainManager::ForceRecheckDomain(d.hostname); break; }
                            }
                            pair.second = false;
                        }
                    }
                ...
                ImGui::SameLine();
                if (ImGui::Button(u8"Нет")) ImGui::CloseCurrentPopup();
                ImGui::EndPopup();
            }
            ImGui::SameLine();

            if (ImGui::Button(u8"Удалить")) ImGui::OpenPopup(u8"ConfirmMassDelete");
            if (ImGui::BeginPopupModal(u8"ConfirmMassDelete", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
                ImGui::Text(u8"Удалить выбранные записи? Это действие необратимо.");
                if (ImGui::Button(u8"Удалить")) {
                    for (auto& pair : selectedRows) {
                        if (pair.second) {
                            for (const auto& d : domains) {
                                if (d.id == pair.first) { DomainManager::DeleteDomain(d.hostname); break; }
                            }
                            pair.second = false;
                        }
                    }
                ...
                ImGui::SameLine();
                if (ImGui::Button(u8"Отмена")) ImGui::CloseCurrentPopup();
                ImGui::EndPopup();
            }

            if (scanningActive) { ImGui::PopItemFlag(); ImGui::PopStyleVar(); }
            ...
            else ImGui::TextDisabled(u8"Нет лога проверки.");
            if (logAutoScroll && ImGui::GetScrollY() >= ImGui::GetScrollMaxY() - 5.0f) ImGui::SetScrollHereY(1.0f);
            if (ImGui::IsWindowHovered() && ImGui::GetIO().MouseWheel != 0.0f) logAutoScroll = false;
            if (ImGui::IsWindowFocused() && !ImGui::IsMouseDragging(ImGuiMouseButton_Left) && ImGui::GetScrollY() < ImGui::GetScrollMaxY() - 5.0f) logAutoScroll = false;
            if (ImGui::GetScrollY() >= ImGui::GetScrollMaxY() - 5.0f) logAutoScroll = true;
            ImGui::EndChild();
...
LRESULT WINAPI WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_USER + 1:
        ...
        else if (lParam == WM_RBUTTONUP) TrayIcon::ShowContextMenu(hWnd);
        break;
    case WM_COMMAND:
        if (LOWORD(wParam) == 101) ZapretController::Restart();
        else if (LOWORD(wParam) == 102) { if (Sniffer::IsRunning()) Sniffer::Stop(); else Sniffer::Start(); }
        else if (LOWORD(wParam) == 103) PostQuitMessage(0);
        break;
    case WM_SYSCOMMAND:
        if ((wParam & 0xfff0) == SC_CLOSE) { ShowWindow(hWnd, SW_HIDE); return 0; }
        break;
    case WM_DESTROY:
        PostQuitMessage(0);
        return 0;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int nCmdShow) {
    ...
    RegisterClassEx(&wc);
    HWND hwnd = CreateWindow(wc.lpszClassName, _T("AdaptiveZapret"), WS_POPUP, CW_USEDEFAULT, CW_USEDEFAULT, 1280, 800, NULL, NULL, wc.hInstance, NULL);
    g_hWnd = hwnd;
    RECT rc; GetWindowRect(hwnd, &rc);
    SetWindowPos(hwnd, NULL, (GetSystemMetrics(SM_CXSCREEN) - (rc.right - rc.left)) / 2, (GetSystemMetrics(SM_CYSCREEN) - (rc.bottom - rc.top)) / 2, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
    if (!CreateDeviceD3D(hwnd)) { CleanupDeviceD3D(); UnregisterClass(wc.lpszClassName, wc.hInstance); WSACleanup(); Database::CloseDB(); return 1; }
    ...
    if (Database::GetZapretState()) {
        OutputDebugStringA("WinMain: Database::GetZapretState true, calling ZapretController::Start\n");
        if (!ZapretController::Start()) {
            OutputDebugStringA("WinMain: ZapretController::Start failed\n");
            MessageBoxA(NULL, "Не удалось запустить ZAPRET автоматически. Проверьте наличие zapret-win-bundle-master.", "Ошибка", MB_ICONWARNING);
        }
    }
    else {
        OutputDebugStringA("WinMain: Database::GetZapretState false, not starting Zapret\n");
    }
    bool dohEnabled = LoadDoHState(); SetSystemDoH(dohEnabled);
    DWORD lastUpdateTime = GetTickCount(); bool done = false;
    const float clear_color[4] = { 0.1f, 0.1f, 0.1f, 1.0f };
    while (!done) {
        MSG msg;
        while (PeekMessage(&msg, NULL, 0U, 0U, PM_REMOVE)) { TranslateMessage(&msg); DispatchMessage(&msg); if (msg.message == WM_QUIT) done = true; }
        if (done) break;
        DWORD now = GetTickCount();
        if (now - lastUpdateTime > 3000) { lastUpdateTime = now; if (!unsavedChanges && DomainManager::HasNewData()) RefreshDomainList(); }
    ...
    // ========== ИСПРАВЛЕННАЯ ЗАВЕРШАЮЩАЯ ЧАСТЬ ==========
    // Сохраняем текущее состояние ZAPRET в БД (без остановки процесса, он умрёт сам)
    Database::UpdateZapretState(ZapretController::IsRunning());
